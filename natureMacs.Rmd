...
```{r}
DimPlot(justMacs.seu, reduction = "tsne")
```
...
```{r}
justMacs.seu.markers <- FindAllMarkers(justMacs.seu, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25, group.by = "group")
```


```{r}
justMacs.seu.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_logFC)
```
```{r}
VlnPlot(justMacs.seu, features = c("Fabp1", "Ear10"), group.by = "group")
```
